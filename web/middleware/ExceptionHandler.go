...
func ExceptionHandler(ctx *gin.Context) {
	defer func() {
		if err := recover(); err != nil {
			...
			ctx.JSON(errCode, gin.H{"msg": errMsg})
			ctx.Abort()
		}
	}()
	ctx.Next()
...
func getError(err interface{}) (int, string) {
	switch v := err.(type) {
		case utils.TraceBackError:
			// 符合预期的错误，可以直接返回给客户端
			return v.Code, v.Msg
		case utils.ServerError:
			// 符合预期的错误，可以直接返回给客户端
			return errorResponse.Error_Server.Code, errorResponse.Error_Server.Msg
		case utils.LoadError:
			// 符合预期的错误，可以直接返回给客户端
			return errorResponse.Error_Load.Code, errorResponse.Error_Load.Msg
		case utils.InferError:
			// 符合预期的错误，可以直接返回给客户端
			return errorResponse.Error_Infer.Code, errorResponse.Error_Infer.Msg
		case error:
